...
def evaluate(posting, parsed=None, seen_keys=None, today=None):
    """Run every gate rule. Returns (allowed, reasons) where reasons is a
    list of human-readable rejection reasons (empty when allowed)."""
    ...
    # 1. Missing required fields -> manual review, never a guess.
    if parsed["flagged"]:
        reasons.append(f"flagged for manual review: missing {', '.join(parsed['flagged'])}")
    ...
    if start:
        ...
        if earliest and start < earliest:
            reasons.append(f"start date {start} before availability window ({earliest})")
        if latest and start > latest:
            reasons.append(f"start date {start} after availability window ({latest})")
    ...
    if (parsed.get("age_days") or 0) > MAX_POSTING_AGE_DAYS:
        reasons.append(f"posting is {parsed['age_days']} days old (> {MAX_POSTING_AGE_DAYS} days)")

    # 6. Duplicate requisition (incl. cross-platform / cross-subsidiary).
    key = _requisition_key(parsed)
    if key in seen_keys:
        reasons.append("duplicate requisition of another posting in this batch")
    else:
        seen_keys.add(key)
...
def annotate(postings):
    """Batch gate. Returns (allowed, blocked) where blocked entries carry a
    `gate_reasons` field, and allowed entries carry a `parsed` field."""
    seen = set()
    allowed, blocked = [], []
    for p in postings:
        ...
        p["parsed"] = parsed
        p["_jd"] = p.get("jd_text", "")
        ok, reasons = evaluate(p, parsed, seen, today=date.today())
        if ok:
            allowed.append(p)
        else:
